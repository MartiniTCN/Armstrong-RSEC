...
// 说明：统一测试页面渲染与评估脚本，支持加载 CSV、评分、语言切换、邮件发送
 ...
 document.addEventListener("DOMContentLoaded", () => {
   ...
   const moon = document.getElementById("moonIcon");
   if (sun && moon) {
     sun.classList.toggle("hidden", savedTheme !== "dark"); // 暗色时显示太阳
     moon.classList.toggle("hidden", savedTheme === "dark"); // 亮色时显示月亮
   }
   ...
   if (themeBtn) {
     themeBtn.addEventListener("click", () => {
       ...
       console.log("[主题切换] 当前模式：", isDark ? "dark" : "light");
     });
   }
 });
