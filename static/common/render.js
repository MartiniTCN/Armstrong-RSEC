...
function switchLanguage(lang) {
  ...
  // Martin console.log(`🌐 已切换语言为：${lang}`);
...
function submitTest() {
  ...
  // Martin console.log("提交时读取的题目数据：", parsedQuestions);
  ...
  // Martin console.log("📤 已收集完成，准备提交答卷");
...
function renderAnswersOnCompletePage(answers) {
  ...
  sortedQuestions.forEach((q, i) => {
    if (!q || !q.type) {
      // Martin console.warn(`⚠️ 第 ${i + 1} 题无效，跳过：`, q);
      return;
    }
  ...
  // Martin console.log("🔄 完成页读取的题目数据：", parsedQuestions);
...
function scrollToType(type) {
  ...
  if (section) {
    ...
    // Martin console.warn("❌ 无法找到题型锚点：", type);
  }
}
...
function updateProgressFromDOM() {
  ...
  function renderDots(selector, type, isAnswered) {
    ...
    // Martin console.log(`正在渲染 ${type} 类型题目的小圆点...`);
    // Martin console.log(`共 ${cards.length} 个题目`);
  ...
  // Martin console.log("🎯 当前各类型答题进度统计", data);
}
...
function collectAnswers() {
  ...
  document.querySelectorAll(".question-card[data-type='single']").forEach(card => {
    ...
    if (id && input) {
      ...
      //// Martin console.warn(`⚠️ 单选题未作答或缺少 ID：`, card);
    }
  });
  ...
  document.querySelectorAll(".question-card[data-type='multiple']").forEach(card => {
    ...
    if (id && checked.length > 0) {
      ...
      //// Martin console.warn(`⚠️ 多选题未作答或缺少 ID：`, card);
    }
  });
  ...
  document.querySelectorAll(".question-card[data-type='judge']").forEach(card => {
    ...
    if (id && input) {
      ...
      //// Martin console.warn(`⚠️ 判断题未作答或缺少 ID：`, card);
    }
  });
  ...
  document.querySelectorAll(".essay-card[data-id]").forEach(card => {
    ...
    if (id && textarea) {
      ...
      //// Martin console.warn(`⚠️ 简答题未作答或缺少 ID：`, card);
    }
  });

  const snapshot = JSON.stringify(answers);
  // Martin console.log(`📦 当前收集到的答题内容：`, snapshot);

  // ✅ 存入全局
  window.collectedAnswers = answers;
  //// Martin console.log("✅ 已收集答题结果：", answers); // Martin 暂时隐藏答题收集结果的 console 提示
  return answers;
...
function initCorrectAnswers(data) {
  ...
  data.forEach((row, idx) => {
    ...
    if (!row || typeof row !== "object") {
      //// Martin console.warn(`❌ 无法识别为有效题目对象，第 ${idx + 1} 行：`, row);
      return;
    }
    
    if (!row.type) {
      // Martin console.warn(`⚠️ 缺少 type 字段，第 ${idx + 1} 行：`, row);
      return;
    }
    
    if (!row.answer || row.answer.trim() === "") {
      // Martin console.warn(`⚠️ 缺少答案字段，第 ${idx + 1} 行：`, row);
      return;
    }
    ...
    if (type === "judge") {
      ...
      if (["true", "yes", "✔", "√", "正确"].includes(normalized)) {
        ...
        // Martin console.warn(`❗️判断题答案格式异常 [index=${idx}]：`, {
          id: row?.id,
  ...
  // ✅ 打印初始化完成信息
  // Martin console.log("✅ 初始化标准答案完成，共计：", Object.keys(correctAnswersMap).length, "题");
...
function restoreUserInfo() {
  ...
  if (!stored) {
    // Martin console.warn("⚠️ 无法恢复用户信息：没有找到缓存");
    return;
  }
  ...
  try {
    ...
    // Martin console.log("✅ 已恢复用户信息", info);
  } catch (e) {
...
function loadCSVAndInit(courseName) {
  ...
  Papa.parse(csvPath, {
    ...
    complete: async function (results) {
      ...
      // Martin console.log(`✅ 题库加载完成，共 ${parsed.length} 题`);
      ...
      try {
        ...
        // Martin console.log("✅ 答题记录已恢复 - loadCSVAndInit");
        ...
        // Martin console.warn("⚠️ DOM 检测超时，跳过答题恢复：", err);
      }
...
async function restoreAnswersFromSupabase() {
  ...
  try {
    ...
    // Martin console.log(`🔄 开始从 Supabase 恢复答题记录，共拉取 ${records.length} 条`);
    ...
    records.forEach(record => {
      ...
      if (question_type === "essay") {
        ...
        if (!card) {
          // Martin console.warn(`❌ 未找到简答题卡片：${question_id}`);
          return;
        }

        const textarea = card.querySelector('textarea');
        if (!textarea) {
          // Martin console.warn(`❌ 卡片 ${question_id} 中未找到 <textarea> 元素`);
          return;
        }

        textarea.value = answer;
        // Martin console.log(`✅ 简答题 ${question_id} 恢复成功：${answer.slice(0, 20)}...`);
        
      } else {
        card = document.querySelector(`.question-card[data-id="${question_id}"][data-type="${question_type}"]`);
        if (!card) {
          // Martin console.warn(`❌ 未找到 ${question_type} 卡片：${question_id}`);
          return;
        }
        ...
        if (question_type === "single" || question_type === "judge") {
          ...
          if (radio) {
            radio.checked = true;
            // Martin console.log(`✅ ${question_type} 题 ${question_id} 恢复成功`);
          }
          ...
          // Martin console.log(`✅ 多选题 ${question_id} 恢复成功`);
        }
      }
    });
    ...
    // Martin console.log("✅ 答题记录已恢复 - restoreAnswersFromSupabase");
...
function renderQuestions(data) {
  ...
  for (const [type, container] of Object.entries(sectionMap)) {
    if (!container) {
      // Martin console.warn(`⚠️ 缺少 ${type} 类型的容器 <div data-section="${type}">，将跳过该类型题目渲染`);
      continue;
    }
  ...
  data.forEach((row, i) => {
    ...
    if (!row || typeof row !== "object") {
      ...
      // Martin console.warn(`⚠️ 第 ${lineNum} 行跳过：行数据为空或格式错误\n📌 字段状态：${fieldStates}`);
      return;
    }

    const requiredFields = ["type", "id", "question_zh", "question_en", "answer"];
    const details = requiredFields.map(f => `${f}: ${row[f] ?? "[缺失]"}`).join(" | ");
    //// Martin console.warn(`🔍 第 ${lineNum} 行字段状态：${details}`);

    const type = row.type?.toLowerCase?.().trim();
    if (!type || !(type in index)) {
      // Martin console.warn(`⚠️ 第 ${lineNum} 行跳过：题型无效或 index 中无对应类型 \"${row.type}\"`, row);
      return;
    }
    ...
    if (!questionText) {
      // Martin console.warn(`⚠️ 第 ${lineNum} 行跳过：无题干（question_zh / question_en）`, row);
      return;
    }
    ...
    try {
      ...
      if (html && sectionMap[type]) {
        ...
        // Martin console.warn(`⚠️ 第 ${lineNum} 行未能渲染至页面（容器缺失或 HTML 为空）`, row);
      }
    } catch (err) {
  ...
  if (missed.length > 0) {
    // Martin console.warn(`📉 共 ${missed.length} 题未被渲染：`, missed);
  }

  // Martin console.log(`✅ 成功渲染题目数：${renderedIds.size}`);
...
function checkMissingFields(row, lineNum, fields) {
  ...
  if (missing.length > 0) {
    try {
      ...
      // Martin console.log(`📌 字段内容：${fieldValues}`);
    } catch (e) {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  themeToggleBtn.addEventListener('click', () => {
    ...
    // Martin console.log(`[Click] 主题切换为 ${isDark ? 'light' : 'dark'}`);
  });
});
...
async function submitAnswerToSupabase({ username, course, questionId, questionType, answer }) {
  if (!username || username === "unknown_user") {
    // Martin console.warn("⚠️ 无法识别用户名，答题记录未上传！");
    return;
  }
  if (!questionId || !questionType || !answer) {
    // Martin console.warn("⚠️ 缺少必要参数，答题记录未上传！");
    return;
  }
  ...
  try {
    ...
    if (!res.ok) {
      // Martin console.warn(`❌ 答案上传失败 (${res.status}):`, await res.text());
    } else {
      // Martin console.log(`✅ 答案已上传或更新：题目 ${questionId}, 类型 ${questionType}, 内容 ${answer}`);
    }
  } catch (err) {
...
async function clearPreviousAnswersFromSupabase(username, course) {
  ...
  try {
    ...
    if (res.ok) {
      // Martin console.log(`🧹 已清除 Supabase 上 ${username} 的 ${course} 旧答题记录`);
    } else {
      // Martin console.warn(`⚠️ 清除答题记录失败：${res.status}`, await res.text());
    }
  } catch (err) {
...
async function loadAnswersFromSupabase() {
  try {
    ...
    const records = await res.json();
    // Martin console.log(`🔄 从 Supabase 恢复答题记录，共拉取 ${records.length} 条`);
